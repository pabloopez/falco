...
static void configure_output_format(falco::app::application &app, falco_engine *engine)
{
	...
	if(app.copts().print_container)
	{
		output_format = "container=%container.name (id=%container.id)";
		replace_container_info = true;
	}
	else if(app.copts().print_kubernetes)
	{
		output_format = "k8s.ns=%k8s.ns.name k8s.pod=%k8s.pod.name container=%container.id";
		replace_container_info = true;
	}
	else if(app.copts().print_mesos)
	{
		output_format = "task=%mesos.task.name container=%container.id";
		replace_container_info = true;
	}
	else if(!app.copts().print_additional.empty())
	{
		output_format = app.copts().print_additional;
...
int falco_init(int argc, char **argv)
{
	...
	try
	{
		...
		if(app.copts().help)
		{
			printf("%s", app.copts().usage().c_str());
			return EXIT_SUCCESS;
		}

		if(app.copts().print_version_info)
		{
		...
		inspector = new sinsp();
		inspector->set_buffer_format(app.copts().event_buffer_format);

		// If required, set the CRI path
		if(!app.copts().cri_socket_path.empty())
		{
			inspector->set_cri_socket_path(app.copts().cri_socket_path);
		}

		// Decide wether to do sync or async for CRI metadata fetch
		inspector->set_cri_async(!app.copts().disable_cri_async);

		//
		// If required, set the snaplen
		//
		if(app.copts().snaplen != 0)
		{
			inspector->set_snaplen(app.copts().snaplen);
		}
		...
		for(const auto &src : app.copts().disable_sources)
		{
		...
		if(app.copts().validate_rules_filenames.size() > 0)
		{
			falco_logger::log(LOG_INFO, "Validating rules file(s):\n");
			for(auto file : app.copts().validate_rules_filenames)
			{
			...
			for(auto file : app.copts().validate_rules_filenames)
			{
				...
				std::string prefix = (app.copts().validate_rules_filenames.size() > 1 ? file + ": " : "");
				try {
					engine->load_rules_file(file, app.copts().verbose, app.copts().all_events);
				}
		...
		falco_configuration config;
		if (app.copts().conf_filename.size())
		{
			config.init(app.copts().conf_filename, app.copts().cmdline_config_options);
			...
			falco_logger::log(LOG_INFO, "Falco initialized with configuration file " + app.copts().conf_filename + "\n");
		}
		else
		{
			throw std::runtime_error("Could not find configuration file at " + app.copts().conf_filename);
		}
		...
		if(!app.copts().list_flds_source.empty())
		{
			list_source_fields(engine, app.copts().verbose, app.copts().names_only, app.copts().list_flds_source);
			return EXIT_SUCCESS;
		}

		if (app.copts().rules_filenames.size())
		{
			config.m_rules_filenames = app.copts().rules_filenames;
		}

		engine->set_min_priority(config.m_min_priority);

		config.m_buffered_outputs = !app.copts().unbuffered_outputs;
		...
		for (auto filename : config.m_rules_filenames)
		{
			...
			try {
				engine->load_rules_file(filename, app.copts().verbose, app.copts().all_events, required_engine_version);
			}
			catch(falco_exception &e)
			{
		...
		for (auto substring : app.copts().disabled_rule_substrings)
		{
			falco_logger::log(LOG_INFO, "Disabling rules matching substring: " + substring + "\n");
			engine->enable_rule(substring, false);
		}

		if(app.copts().disabled_rule_tags.size() > 0)
		{
			for(auto &tag : app.copts().disabled_rule_tags)
			{
				falco_logger::log(LOG_INFO, "Disabling rules with tag: " + tag + "\n");
			}
			engine->enable_rule_by_tag(app.copts().disabled_rule_tags, false);
		}

		if(app.copts().enabled_rule_tags.size() > 0)
		{

			// Since we only want to enable specific
			// rules, first disable all rules.
			engine->enable_rule(all_rules, false);
			for(auto &tag : app.copts().enabled_rule_tags)
			{
				falco_logger::log(LOG_INFO, "Enabling rules with tag: " + tag + "\n");
			}
			engine->enable_rule_by_tag(app.copts().enabled_rule_tags, true);
		}

		if(app.copts().print_support)
		{
			...
			support["config"] = read_file(app.copts().conf_filename);
		...
		if (app.copts().describe_all_rules)
		{
			engine->describe_rule(NULL);
			goto exit;
		}

		if (!app.copts().describe_rule.empty())
		{
			engine->describe_rule(&(app.copts().describe_rule));
			goto exit;
		}
		...
		if (app.copts().daemon && !g_daemonized) {
			pid_t pid, sid;
			...
			if (pid < 0) {
				...
				pidfile.open(app.copts().pidfilename);

				if (!pidfile.good())
				{
					falco_logger::log(LOG_ERR, "Could not write pid to pid file " + app.copts().pidfilename + ". Exiting.\n");
					result = EXIT_FAILURE;
					goto exit;
				}
				pidfile << pid;
		...
		if(app.copts().trace_filename.size())
		{
			// Try to open the trace file as a
			// capture file first.
			try {
				inspector->open(app.copts().trace_filename);
				falco_logger::log(LOG_INFO, "Reading system call events from file: " + app.copts().trace_filename + "\n");
			}
			catch(sinsp_exception &e)
			{
				falco_logger::log(LOG_DEBUG, "Could not read trace file \"" + app.copts().trace_filename + "\": " + string(e.what()));
				trace_is_scap=false;
			}
			...
			if(!trace_is_scap)
			{
				...
				try {
					...
					ifstream ifs(app.copts().trace_filename);
					getline(ifs, line);
					j = nlohmann::json::parse(line);

					falco_logger::log(LOG_INFO, "Reading k8s audit events from file: " + app.copts().trace_filename + "\n");
				}
				catch (nlohmann::json::parse_error& e)
				{
					fprintf(stderr, "Trace filename %s not recognized as system call events or k8s audit events\n", app.copts().trace_filename.c_str());
					result = EXIT_FAILURE;
					goto exit;
				}
				catch (exception &e)
				{
					fprintf(stderr, "Could not open trace filename %s for reading: %s\n", app.copts().trace_filename.c_str(), e.what());
					result = EXIT_FAILURE;
					goto exit;
				}
#endif
		...
		else
		{
			...
			catch(sinsp_exception &e)
			{
				...
				if (enabled_sources.find(syscall_source) == enabled_sources.end() && !app.copts().userspace)
				{
		...
		// Run k8s, if required
		//
		if(!app.copts().k8s_api.empty())
		{
			// Create string pointers for some config vars
			// and pass to inspector. The inspector then
			// owns the pointers.
			std::string *k8s_api_ptr = new string(app.copts().k8s_api);
			std::string *k8s_api_cert_ptr = new string(app.copts().k8s_api_cert);
			std::string *k8s_node_name_ptr = new string(app.copts().k8s_node_name);
			...
			inspector->init_k8s_client(k8s_api_ptr, k8s_api_cert_ptr, k8s_node_name_ptr, app.copts().verbose);
		}
		...
		if(!app.copts().mesos_api.empty())
		{
			...
			inspector->init_mesos_client(&(app.copts().mesos_api), app.copts().verbose);
		}
		else if(char* mesos_api_env = getenv("FALCO_MESOS_API"))
		{
			std::string mesos_api_copy = mesos_api_env;
			inspector->init_mesos_client(&mesos_api_copy, app.copts().verbose);
		}
		...
		if(app.copts().trace_filename.empty() && config.m_webserver_enabled && enabled_sources.find(k8s_audit_source) == enabled_sources.end())
		{
		...
		if(!app.copts().trace_filename.empty() && !trace_is_scap)
		{
#ifndef MINIMAL_BUILD
			read_k8s_audit_trace_file(engine,
						  outputs,
						  app.copts().trace_filename);
#endif
		}
		else
		{
			...
			num_evts = do_inspect(engine,
					      ...
					      config,
					      sdropmgr,
					      uint64_t(app.copts().duration_to_tot*ONE_SECOND_IN_NS),
					      app.copts().stats_filename,
					      app.copts().stats_interval,
					      app.copts().all_events,
					      result);
		...
		if(!app.copts().trace_filename.empty() && app.copts().duration_to_tot>0)
		{
			std::this_thread::sleep_for(std::chrono::seconds(app.copts().duration_to_tot));
		}
